...
namespace Residence_Web_Scraper.HandlerClasses
{
    public static class DataBaseHandler // add Backup and LoadBackup
    {
        ...
        private static readonly string ConnectionString = "Data Source=.\\ResidencesDatabase.db;Version=3;";
        ...
        public static async Task AddResidences(List<Residence> Residences)
        {
            //wait for a time when no methods writes now to the database
            await _semaphore.WaitAsync();
            //if no one writes continue
                    ...
                    await Task.Run(async() =>
                    {
                        using (SQLiteTransaction transaction = connection.BeginTransaction())
                        {
                            ...
                            try
                            {
                                ...
                                foreach (Residence res in Residences)
                                {
        ...
        public static async Task<List<Residence>> GetFilteredResidences( int SelectedMinPrice,int SelectedMaxPrice,string City,string County,string ResidenceType,string BuyOrRent,string OrderBy,string Order,int MaxVisibleRes)
        {
            //wait for a time when no methods writes now to the database
            await _semaphore.WaitAsync();
            //if no one writes continue
        ...
        public static async Task<List<Residence>> GetFavResidences(int Offset, int SelectedMinPrice, int SelectedMaxPrice, string City, string County, string ResidenceType, string BuyOrRent, string Order, int MaxVisibleRes)
        {
            //wait for a time when no methods writes now to the database
            await _semaphore.WaitAsync();
            //if no one writes continue
        ...
        public static async Task<List<(Residence,int)>> GetChangedResidences(int MaxVisibleRes)
        {
            //wait for a time when no methods writes now to the database
            await _semaphore.WaitAsync();
            //if no one writes to db continue
            ...
            List<(Residence, int)> ReturnRes = new();
            try 
            {
        ...
        public static async Task AddFavResidence(Residence res)
        {
            //wait for a time when no methods writes now to the database
            await _semaphore.WaitAsync();
            try
